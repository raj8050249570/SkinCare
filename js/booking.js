/**
 * RADIANCE SKIN CLINIC - Appointment Booking & WhatsApp Integration
 */
...
const CLINIC_WHATSAPP_NUMBER = "919876543210"; // Placeholder clinic number (easy to replace)
...
function initBookingForm() {
  ...
  form.addEventListener('submit', (e) => {
    ...
    if (confirmModal) {
      ...
      const waMsg = `Hello Radiance Skin Clinic, I would like to confirm my consultation for ${treatment} on ${date}. My name is ${name} (${phone}).`;
...
function initWhatsAppDirectButtons() {
  const defaultMsg = "Hello Radiance Skin Clinic, I would like to book a skin consultation.";
  const defaultUrl = `https://wa.me/${CLINIC_WHATSAPP_NUMBER}?text=${encodeURIComponent(defaultMsg)}`;

  const waButtons = document.querySelectorAll('.whatsapp-trigger, .floating-whatsapp-btn');
